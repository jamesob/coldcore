...
def _prepare_send(
    ...
    # Altered code from confirm_broadcast to display inputs and outputs
    psbtinfo = info # BG edit
    tx = info["tx"] # BG edit
    inputs = tx["vin"] # BG edit
    outputs = tx["vout"] # BG edit
    outs = outputs # BG edit
    total_in_amt = 0 # BG edit
    total_out_amt = 0 # BG edit
    change = 0
    ...
    #     # TODO: Does 'addresses' always contain one address?
    #     address = output['scriptPubKey']['addresses'][0]
    #     F.blank(f"  {yellow(address)}: {str(output['value'])} BTC")
    # F.p()
    # Display outputs
    for o in outs:
        addr = o['scriptPubKey']['addresses'][0]
